...
import type { Wall, WallId, AmmoType, Personnel } from '@/engine/types'
...
export function WeaponDashboard({ walls, ammoInventory, personnel, onLoadAmmo }: WeaponDashboardProps) {
  const weapons = Object.values(walls)
    .flatMap((wall) =>
      wall.weapons
        .filter((w): w is NonNullable<typeof w> => w !== null && w.exists)
        .map((w) => ({ weapon: w, wallLabel: wall.id }))
    )
            ...
            <CardContent className="py-3 px-3 space-y-2">
              <div className="flex items-center justify-between">
                <span className="font-semibold text-sm capitalize">
                  {wallLabel} · Slot {weapon.slot + 1}
                </span>
                <span className="font-mono text-xs text-muted-foreground">{weapon.id.slice(-4)}</span>
              </div>
              ...
              <div className="space-y-1">
                ...
                <Progress value={durPct} className={cn('h-2', durIndicatorClass(weapon.durability))} />
              </div>
              ...
              {assignedPersonnel.length > 0 && (
                <p className="text-xs text-muted-foreground">
                  Manned: {assignedPersonnel.map((p) => `Soldier ${p.id + 1} (${p.mode})`).join(', ')}
                </p>
              )}
